"""FastAPI server for Guide ChatKit backend.

Run:
    cd sample
    uv run uvicorn guide.server:app --reload --port 8000
"""
...
class GuideServer(ChatKitServer[dict[str, Any]]):
    """ChatKit server for AF Guide."""
    ...
    async def respond(
        ...
        runner = Runner(flow=guide_flow, session=session)
